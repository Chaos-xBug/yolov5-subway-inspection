# -*- coding: utf-8 -*-
# @Time : 2022/3/31 下午14:31
# @Author : iano
# @File : main01.py
# @desc: 由待检照片匹配模板，改为模板匹配待检图片，从而判断无法覆盖项点清单，及检测缺失清单中项点在待检图片中的坐标。
import numpy as np
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(os.path.join(path_base, "Defaut")):
        ...
        save_path = os.path.join(path_base, "manques\\", filename)
        ...
        m, n = img_o.shape
        kp1, kp2, matches = Compute_Sift(img_check, img_o)
        warped, Minv = Transform_Sansac(img_check, img_o, kp1, kp2, matches)
        # try:
        #     if warped == -1:
        #         continue
        # except:
        #     print(count)
        plt.imshow(img_check, 'gray'), plt.title('Test Image'), plt.show()
        plt.imshow(warped, 'gray'), plt.title('Warped Standard Image'), plt.show()
        ...
        gt_boxes = np.zeros(gt_boxes_.shape)
        gt_boxes[:, 2:4] = gt_boxes_[:, 2:4]
        for i in range(gt_boxes_.shape[0]):
            pts_ = np.float32(gt_boxes_[i, 0:2]).reshape(-1, 1, 2)
            ddd = cv2.perspectiveTransform(pts_, Minv)  # 坐标变换
            gt_boxes[i, 0:2] = ddd.reshape(1, 2)

            # 通过画图表示前后变换的框图位置对比验证
            w = gt_boxes_[i, 2]
            h = gt_boxes_[i, 3]
            cv2.rectangle(img_o, (int(gt_boxes_[i, 0] - w / 2), int(gt_boxes_[i, 1] - h / 2)),
                          (int(gt_boxes_[i, 0] + w / 2), int(gt_boxes_[i, 1] + h / 2)), (0, 0, 255), 10)
            cv2.rectangle(warped, (int(gt_boxes[i, 0] - w / 2), int(gt_boxes[i, 1] - h / 2)),
                          (int(gt_boxes[i, 0] + w / 2), int(gt_boxes[i, 1] + h / 2)), (0, 0, 255), 10)
        # 画图检查
        # plt.imshow(img_o, 'gray'), plt.title('standard image with boxes'), plt.show()
        ...
        for i in range(gt_boxes.shape[0]):
            if gt_boxes[i, 0] < 30 or gt_boxes[i, 0] > 5090 or gt_boxes[i, 1] < 30 or gt_boxes[i, 1] > 5090: # 应该添加w h
        ...
        for i in range(n2):
            for j in range(n1):
                IOU[i, j] = compute_iou(det_boxes[i, :], coverlist[j, :])
        ...
        for i in range(len(iou_max)):
            if iou_max_index[i] == False:
                w = coverlist[i, 2]
                h = coverlist[i, 3]
                roi = img_check3[int(coverlist[i, 1] - h / 2)-15:int(coverlist[i, 1] + h / 2)+15, int(coverlist[i, 0] - w / 2)-15:int(coverlist[i, 0] + w / 2)+15]
                ...
                # print(w)
                # print('roi:',roi)
                plt.imshow(roi), plt.title(classes[int(label_o[i])]), plt.show()
                cv2.imwrite(save_path + "\\"+ str(i) + classes[int(label_o[i])] + '-check.jpg', roi)
                roi_ = warped[int(coverlist[i, 1] - h / 2):int(coverlist[i, 1] + h / 2),
                      int(coverlist[i, 0] - w / 2):int(coverlist[i, 0] + w / 2)]
                plt.imshow(roi_, 'gray'), plt.title(classes[int(label_o[i])]), plt.show()
                cv2.imwrite(save_path + "\\"+ str(i) +classes[int(label_o[i])]+ '-warped.jpg', roi_)
