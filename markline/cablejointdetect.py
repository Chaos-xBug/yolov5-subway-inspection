...
def find_line(image):
    ...
    old_img, close, contours, num, old_img1, dst_img, hsv_img, mask, open = detect_cj_line(image)
    ...
    while i < num:
        ...
        s = area_con / area1
        if s >= 0.01 and (wh_ratio > 1.1 or hw_ratio > 1.1):
...
def CableJoint_State(image):
    ...
    if num == 0:
        ...
        # 由于cable joint的图片中有一种会有一根垂直的红线，所以增加一点点判定条件
        if anglestate == 1 and pointstate == 1:
            state = 1
            # print("没有松动", "角度差：", included_angle, "相对距离差：", min_Dis)
        else:
            if 79 < included_angle < 101:
                state = 1
                # print("没有松动", "角度差：", included_angle, "相对距离差：", min_Dis, "这里可能是有那根垂直的红线")
            else:
                state = 0
                # print("松动了", "角度差：", included_angle, "相对距离差：", min_Dis)

    # 三条线的还需要进一步研究，目前延续使用螺栓最小角度差和距离差的方法
    elif num == 3:
...
if __name__ == "__main__":
    ...
    root_path = "/Users/kerwinji/Desktop/tagline_detect/cablejoint_result/59/sd"
    image_path = os.path.join(root_path, "CableJoint10_Pic_2022_05_12_153931_231.bmp")
    image = cv2.imread(image_path)

    old_img, close, contours1, num, old_img1, dst_img, hsv_img, mask, open = detect_cj_line(image)
    ...
    state, old_img3 = CableJoint_State(image)
