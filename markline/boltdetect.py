...
def find_line(image):
    ...
    old_img, close, contours, num, old_img1, dst_img, hsv_img, mask, open = detect_bolt_line(image)
    ...
    while i < num:
        ...
        s = area_con / area1
        if s >= 0.003 and (wh_ratio > 1.1 or hw_ratio > 1.1):
...
def Bolt_State(image):
    ...
    # 实际的场景中很难检测到三个正确的轮廓（螺栓、垫片、背景）
    # 所以这里暂时不考虑垫片上的标记线，而是将垫片上的标记线与有可能检测到的其他背景中的红色杂质都看成是误检出的标记线
    # 所以当下的逻辑理论上来说是会造成误检的
    # 逻辑：角度差里最小的视作螺栓上和对应背景中的标记线的角度差，距离同理
    elif num == 3:
...
if __name__ == "__main__":
    ...
    root_path = "/Users/kerwinji/Desktop/tagline_detect/bolt_result/39/sd"
    image_path = os.path.join(root_path, "Bolt40_Pic_2022_05_12_145859_28.bmp")
    image = cv2.imread(image_path)

    old_img, close, contours1, num, old_img1, dst_img, hsv_img, mask, open = detect_bolt_line(image)
    ...
    state, old_img3 = Bolt_State(image)
